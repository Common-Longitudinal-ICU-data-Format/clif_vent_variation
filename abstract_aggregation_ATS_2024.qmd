...
combined_model_coefs_sex_plot <-  combined_model_coefs_sex |> 
...
# Getting male estimates
male_estimates <- combined_model_coefs_sex |> 
filter(Variable == "sex_categorymale") |> 
filter(Site != "EU") |> 
  summarise(mean_male_estimate = mean(Estimate, na.rm = TRUE),
    min_male_estimate = min(Estimate, na.rm = TRUE),
    max_male_estimate = max(Estimate, na.rm = TRUE),
         .by = Model_Simple) 


male_estimates

significant_m_vt <- combined_model_coefs_sex |> 
  filter(Variable == "sex_categorymale") |> 
  filter(Site != "EU") |> 
...
significant_m_h_vt <- combined_model_coefs_sex |> 
  filter(Variable == "sex_categorymale") |> 
  filter(Site != "EU") |> 
...
#mediation analysis
coef_m_vt <- combined_model_coefs_sex |> 
  filter(Variable == "sex_categorymale") |> 
  filter(Site != "EU") |> 
  filter(Model_Simple == "m_vt") |> 
  select(est_m_vt = Estimate, Site)

coef_m_h_vt <- combined_model_coefs_sex |> 
  filter(Variable == "sex_categorymale") |> 
  filter(Site != "EU") |> 
  filter(Model_Simple == "m_h_vt") |>  
...
all_predictions_continuous_plot <- all_predictions_continuous |> 
  ...
  geom_line(size = 1) +
  # facet_wrap(~ Model) +
    facet_grid(health_system ~ Model) +
  labs(
