...
Estimates were significant for female (vs. male) in **`r significant_f_vt`** hospitals and ranged from **`r round(male_estimates |> filter(Model_Simple == "f_vt") |> select(min_male_estimate), 1)`** to **`r round(male_estimates |> filter(Model_Simple == "f_vt") |> select(max_male_estimate), 1)`**, but when corrected for height the estimates were significant in **`r significant_f_h_vt`** hospitals and ranged from **`r round(male_estimates |> filter(Model_Simple == "f_h_vt") |> select(min_male_estimate), 1)`** to **`r round(male_estimates |> filter(Model_Simple == "f_h_vt") |> select(max_male_estimate), 1)`**.
