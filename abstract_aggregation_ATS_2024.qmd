...
extract_base_name <- function(file_path) {
  ...
  if (length(parts) < 2) {
    warning(paste("Filename does not contain double underscore '__':", file_name))
...
hospital_data <- lapply(hospital_files, function(file) {
...
hospital_table1_summary_clean <- hospital_table1_summary  |> 
    mutate(system_hospital = paste("System", Health_system_ID, "\n Hosp", Hospital_ID),
         system = paste("System", Health_system_ID)) |> 
  ...
  select(
    ...
    `Ventilator Days (median [IQR])` = Ventilator.Days,
    `Mortality (%)` = Mortality,
    `System-Hospital` = system_hospital
  ) |>
  arrange(`System-Hospital`) |>   # <--- THIS LINE SORTS BY FIRST COLUMN
  
  gt(rowname_col = "System-Hospital") |> 
  fmt_markdown(columns = "System-Hospital") |> 
  # Bold row labels
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_stub()
  ) |> 
  # Bold only the variable name in column labels (before parentheses)
  cols_label(
    `Sample Size` = md("**Sample Size**"),
    `Age (mean ± SD)` = md("**Age** (mean ± SD)"),
    `Female (%)` = md("**Female** (%)"),
    `Black (%)` = md("**Black** (%)"),
    `White (%)` = md("**White** (%)"),
    `Asian (%)` = md("**Asian** (%)"),
    `Other Race (%)` = md("**Other Race** (%)"),
    `Hispanic (%)` = md("**Hispanic** (%)"),
    `Ventilator Days (median [IQR])` = md("**Ventilator Days** (median [IQR])"),
    `Mortality (%)` = md("**Mortality** (%)")
  ) |> 
  tab_header(
    title = md("**Hospital-Level Summary Table**")
  ) |> 
...
hospital_table1_summary_clean

hospital_table1_summary |> 
 writexl::write_xlsx("aggregate_data/presentation_outputs/descriptive_tables/hospital_summary_table.xlsx")
...
model_names <- c(
  "Female ~ HTVV (Binary)"                = "F_HTVV",
  "Female ~ Vt"                           = "F_Vt",
  "Female + Height ~ HTVV (Binary)"       = "F_H_HTVV",
  "Female + Height ~ Vt"                  = "F_H_Vt",
  "Female * Height ~ HTVV (Binary)"       = "F_Hx_HTVV",
  "Female * Height ~ Vt"                  = "F_Hx_Vt",
  "Female * rcs(Height) ~ HTVV (Binary)"  = "F_rcsHx_HTVV",
  "Female * rcs(Height) ~ Vt"             = "F_rcsHx_Vt"
)
...
combined_model_coefs_sex_clean <- combined_model_coefs_sex |> 
  ...
  mutate(
    Model_Simple = factor(Model, levels = c(
      "Female ~ HTVV (Binary)",
      "Female + Height ~ HTVV (Binary)",
      "Female * Height ~ HTVV (Binary)",
      "Female * rcs(Height) ~ HTVV (Binary)",
      "Female ~ Vt",
      "Female + Height ~ Vt",
      "Female * Height ~ Vt",
      "Female * rcs(Height) ~ Vt"
    ),
                          labels = c(
                            "f_htvv", 
                            "f_h_htvv", 
...
significant_f_vt <- combined_model_coefs_sex_clean |> 
  filter(Variable == "sex_categoryfemale") |> 
  filter(Site != "EU") |> 
  filter(Model_Simple == "f_vt") |> 
  filter(significant == "Yes") |> 
  nrow() 

significant_f_h_vt <- combined_model_coefs_sex_clean |> 
  filter(Variable == "sex_categoryfemale") |> 
  filter(Site != "EU") |> 
  filter(Model_Simple == "f_h_vt") |> 
  filter(significant == "Yes") |> 
  nrow() 


#mediation analysis
coef_f_vt <- combined_model_coefs_sex_clean |> 
  filter(Variable == "sex_categoryfemale") |> 
  filter(Site != "EU") |> 
  filter(Model_Simple == "f_vt") |> 
  select(est_f_vt = Estimate, Site)

coef_f_h_vt <- combined_model_coefs_sex_clean |> 
  filter(Variable == "sex_categoryfemale") |> 
  filter(Site != "EU") |> 
  filter(Model_Simple == "f_h_vt") |>  
  select(est_f_h_vt = Estimate, Site)

mediation_vt <- coef_f_h_vt |> left_join(coef_f_vt) |> 
  mutate(difference = est_f_vt - est_f_h_vt) |> 
  mutate(mediation = difference / est_f_vt)
...
all_predictions_continuous_plot <- all_predictions_continuous |> 
  ...
  labs(
    title = "Predicted Tidal Volume/PBW by Height and Sex Category",
    x = "Height (inches)",
...
all_predictions_binary_plot <- all_predictions_binary |> 
  ...
  facet_grid(health_system ~ Model,  scales = "free_y") +
  labs(
    title = "Predicted Rate of HTVV by Height and Sex Category",
    x = "Height (inches)",
    y = "Rate of HTVV (>8cc/kg PBW)",
    color = "Sex Category",
    fill = "Sex Category"
  ) +
    scale_x_continuous(
...
Estimates were significant for female (vs. male) in **`r significant_f_vt`** hospitals and ranged from **`r round(female_estimates |> filter(Model_Simple == "f_vt") |> select(min_female_estimate), 1)`** to **`r round(female_estimates |> filter(Model_Simple == "f_vt") |> select(max_female_estimate), 1)`**, but when corrected for height the estimates were significant in **`r significant_f_h_vt`** hospitals and ranged from **`r round(female_estimates |> filter(Model_Simple == "f_h_vt") |> select(min_female_estimate), 1)`** to **`r round(female_estimates |> filter(Model_Simple == "f_h_vt") |> select(max_female_estimate), 1)`**.
...
mode_hosp_order3 <- mode_hourly_resp_support_table |> 
  mutate(total_mode_hours = sum(n, na.rm = TRUE),
         .by = hospital_id_graph) |> 
  filter(mode_category %in% c("assist control-volume control","pressure-regulated volume control")) |> 
  distinct() |>
...
mort_htvv <- landmark_files_mort %>%
  ...
  mutate(landmark_day = landmark_hour/24) |> 
  filter(landmark_day != 28) 


ggplot(mort_htvv, aes(x = as.numeric(landmark_day), y = estimate, color = site)) +
  geom_line() +
  geom_point() +
...
vfd_htvv <- landmark_files_vfds %>%
    mutate(landmark_day = landmark_hour/24) |> 
  filter(term == "htvv_cumulative") |> 
  rename(estimate_main = estimate, std_error_main = std.error)

# This is just htvv_cumulative for severe
ggplot(vfd_htvv, aes(x = as.numeric(landmark_day), y = estimate_main, color = site)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.3) +
  facet_wrap(~ site, scales = "fixed") +
...
# Getting marginal effects for everything
vfd_marginal <- landmark_files_vfds %>%
  filter(str_detect(term, "htvv_cumulative:sf_category")) %>%
  mutate(sf_category = str_remove(term, "htvv_cumulative:")) %>%
  select(landmark_hour, sf_category, estimate_interaction = estimate, std_error_interaction = std.error, site) %>%
  left_join(vfd_htvv, by = c("landmark_hour", "site")) %>%
  mutate(
  ...
  select(landmark_hour, sf_category, marginal_estimate, conf.low, conf.high, model, estimate_type, site)
...
vfd_effect <- bind_rows(vfd_marginal, vfd_severe) |> 
  mutate(sf_category = factor(sf_category,
  levels = c("Severe", "sf_categoryModerate", "sf_categoryMild_Normal"),
  labels = c("Severe", "Moderate", "Mild or Normal"),
  ordered = TRUE
)) |> 
  filter(sf_category %in% c(
    "Severe",
    "Moderate",
    "Mild or Normal"
  )) |> 
    filter(landmark_hour != 672) |> 
  filter(landmark_hour != 24)



combined_effect <- bind_rows(mort_htvv, vfd_effect) |> 
  filter(landmark_hour != 672)  |> 
  filter(landmark_hour != 24) 

# Plot
ggplot(vfd_effect, aes(x = landmark_hour / 24, y = marginal_estimate, color = sf_category)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.3) +
...
#EEFFECTS
# Get all objects in the environment that start with "model_coefs_sex"
model_objects <- mget(ls(pattern = "^ALL_model_coefs_landmark_effects_vfds"))

# Combine all objects into one dataset
landmark_files_effects_vfds <- bind_rows(model_objects, .id = "source")



# Make sure the landmark hour is expressed in days for readability
effects_vfd_df_plot <- landmark_files_effects_vfds %>%
  mutate(landmark_day = as.numeric(landmark_hour) / 24) |> 
  filter(landmark_hour != 672) |> 
  group_by(landmark_day, x, group) %>%
  summarise(
