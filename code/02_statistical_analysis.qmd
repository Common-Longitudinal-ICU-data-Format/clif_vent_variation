...
write.csv(sbt, paste0("output/intermediate/sbt__",site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
...
write.csv(mode_hospital_summary_table, paste0("output/final/mode_hospital_summary_table__",site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
...
### A FINAL TABLE
write.csv(mode_hourly_resp_support_table, paste0("output/final/mode_hourly_resp_support_table__",site_name, "_", Sys.Date(),".csv"), row.names = FALSE, fileEncoding = "UTF-8")
...
### A FINAL TABLE
write.csv(ltvv_variation_table, paste0("output/final/ltvv_variation_table__",site_name,"_", Sys.Date(),".csv"), row.names = FALSE, fileEncoding = "UTF-8")
```
...
#   write.csv(df, paste0("output/intermediate/table1_", hosp, '.csv'), row.names = FALSE, fileEncoding = "UTF-8")
#   
# }
...
write.csv(table_summary, paste0("output/final/hospital_table1_summary__", site_name,"_", Sys.Date(),".csv"), row.names = FALSE, fileEncoding = "UTF-8")
...
### A FINAL TABLE
write.csv(ltvv_female_table, paste0("output/final/ltvv_female_table__",site_name,"_", Sys.Date(),".csv"), row.names = FALSE, fileEncoding = "UTF-8")
...
df_oxygenation_index_analysis_prep1 <- df_hourly_resp_support |>
  ...
  mutate(vent_01 = fcase(device_category %in% c("imv", "trach collar"), 1,
                         tracheostomy == 1, 1,
                         default = 0)) |> 
...
# write.csv(model_results, paste0("output/final/logistic_regression_results_.", site_name,"_", Sys.Date(),".csv"), row.names = FALSE, fileEncoding = "UTF-8")
# 
# 
...
# write.csv(model_interaction_results, paste0("output/final/logistic_regression_interaction_results_.", site_name,"_", Sys.Date(),".csv"), row.names = FALSE, fileEncoding = "UTF-8")
# 
...
run_analysis_for_variable_finalfit <- function(var_name, interaction_term = FALSE) {
  ...
  model_data <- delta_var %>%
    left_join(first_intubation_data %>%
                group_by(clif_hospitalizations_joined_id) %>%
                dplyr::summarize(mortality_enc = ffirst(mortality_enc),
  ...
  suffix <- ifelse(interaction_term, "interaction", "no_interaction")
  write.csv(tidy_fit, file = paste0("output/intermediate/model_coefs__", var_name, "_", suffix, "__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
...
### a way to test var_daily that is in the function below ##
  ...
  var_daily_testing <- intubation_data %>%
    ...
    dplyr::summarize(laps2_daily =  fmax(laps2, na.rm = TRUE)) %>%
...
## Daily Analysis with LAPS2/SF etc...
```{r}
# Updated function
run_daily_analysis_for_variable_finalfit <- function(var_name, agg_func, interaction_term = FALSE) {
  ...
  ventilator_hours <- intubation_data %>%
    group_by(clif_hospitalizations_joined_id) %>%
    dplyr::summarize(total_ventilator_hours = sum(vent_01, na.rm = TRUE)) %>%
  ...
  var_daily <- intubation_data %>%
    group_by(clif_hospitalizations_joined_id, day, htvv_daily_percent) %>%
    dplyr::summarize(!!var_daily_name := agg_func(get(var_name), na.rm = TRUE)) %>%
    ungroup() |> 
    arrange(clif_hospitalizations_joined_id, day) %>%
    group_by(clif_hospitalizations_joined_id) %>%
    # Handle missing data by carrying forward previous day's value
 ...
 model_data$sex_category <- factor(model_data$sex_category, levels = c("Male", "Female"))
...
#
  ...
  write.csv(tidy_fit, file = coefficients_filename, row.names = FALSE, fileEncoding = "UTF-8")
...
We defined ventilator-free days (VFDs) as the number of days between successful extubation and day 28, provided the patient remained alive and free of reintubation. If the patient died before day 28 or was reintubated, VFDs were assigned as zero
## Landmark Analysis
```{r}
# Landmark Analysis Function
run_daily_landmark_analysis_for_variable_finalfit <- function(landmark_hours = c(
  24,
  48,
  72,
  96,
  ...
  message("Running landmark analysis for sf_ratio and laps2")
  
  results_mort_list <- list()
  results_vfd_list <- list()
  results_effects_vfd_list <- list()

  for (landmark_hour in landmark_hours) {

    message(paste("Processing landmark day:", landmark_hour/24))


landmark_day = landmark_hour/24

    # Summarize variables up to and on the landmark day
    max_hours <- intubation_data %>%
      # filter(!is.na(tidal_volume_set)) |> 
      group_by(clif_hospitalizations_joined_id) %>%
      dplyr::summarize(max_day = fmax(day, na.rm = TRUE),
                mortality_enc_full = ffirst(mortality_enc),
                max_hour_enc = fmax(hour_sequence_from_first_vent_to_discharge)) |> 
      ungroup() |> 
      mutate(death_hour_seq = fifelse(mortality_enc_full == 1, max_hour_enc, 9999999))    
    
    max_hours_vent <- intubation_data %>%
      filter(vent_01 == 1) |>
      ###  we are including all episodes because if you get extubated and reintubated... most will found these as not being VFHs
      ###      you need to be extubated successfully and alive
      # filter(vent_episode_id == 1) |> 
      group_by(clif_hospitalizations_joined_id) %>%
      dplyr::summarize(max_hour_vent = max(hour_sequence_from_first_vent_to_discharge, na.rm = TRUE)) |> 
      ungroup() 

    landmark_data <- intubation_data %>%
      # filter(!is.na(tidal_volume_set)) |> 
      filter(hour_sequence_from_first_vent_to_discharge <= landmark_hour) |> 
      left_join(max_hours, by = "clif_hospitalizations_joined_id") |> 
      left_join(max_hours_vent, by = "clif_hospitalizations_joined_id") |> 
      group_by(clif_hospitalizations_joined_id, day) |> 
      mutate(
        VFHs = case_when(
          death_hour_seq < landmark_hour ~ 0,
          death_hour_seq > landmark_hour & death_hour_seq <= (landmark_hour + 672) ~ 0,
          max_hour_vent >= (landmark_hour + 672) ~ 0,
          max_hour_vent < (landmark_hour + 672) ~ 672 - (max_hour_vent - landmark_hour),
          TRUE ~ NA
        )
      ) |>  
      ungroup() |> 
      ...
      mutate(
        ...
        hospital_id = str_to_title(hospital_id)
      )
    
    severity_of_illness_vars <- landmark_data |> 
      group_by(clif_hospitalizations_joined_id) |>      
          mutate(
        enc_mean_sf_ratio = if (all(is.na(sf_ratio))) NA_real_ else fmean(sf_ratio, na.rm = TRUE), # some days only have a few hours (the day they die... need NAs)
        enc_mean_laps2 = fmean(laps2, na.rm = TRUE)) |> 
      ungroup() |> 
      filter(hour_sequence_from_first_vent_to_discharge <= 24) |> 
      group_by(clif_hospitalizations_joined_id) |> 
      mutate(
              admit_sf_ratio_worst = if (all(is.na(sf_ratio))) NA_real_ else fmin(sf_ratio, na.rm = TRUE), # some days only have a few hours (the day they die... need NAs)
        admit_laps2_worst = fmax(laps2, na.rm = TRUE)) |> 
      ungroup() |> 
      select(clif_hospitalizations_joined_id, enc_mean_sf_ratio, enc_mean_laps2, admit_sf_ratio_worst, admit_laps2_worst) |> 
      distinct()
        
    model_data_prep <- landmark_data |> 
      group_by(clif_hospitalizations_joined_id) |> 
      mutate(
        htvv_count = sum(htvv, na.rm = TRUE),
        htvv_eligible = sum(!is.na(htvv), na.rm = TRUE),
        htvv_cumulative_mean = if (all(is.na(htvv_daily_percent))) 0 else mean(htvv_daily_percent, na.rm = TRUE),
        
      ) |> 
      ungroup() |> 
      group_by(clif_hospitalizations_joined_id, day) |> 
      
      # drop trach data... they will be in the data until they are trach'd
      filter(is.na(tracheostomy)) |>
      # drop those where VFHs is negative because that means they are extubated
      filter(VFHs >= 0) |> 
      dplyr::summarize(
        # enc_mean_sf_ratio = ffirst(enc_mean_sf_ratio, na.rm = TRUE),
        # enc_mean_laps2 = ffirst(enc_mean_laps2, na.rm = TRUE),
        daily_sf_ratio = if (all(is.na(sf_ratio))) NA_real_ else fmin(sf_ratio, na.rm = TRUE), # some days only have a few hours (the day they die... need NAs)
        daily_laps2 = fmax(laps2, na.rm = TRUE),
        htvv_cumulative = ffirst(htvv_count, na.rm = TRUE),
        htvv_eligible = ffirst(htvv_eligible, na.rm = TRUE),
        htvv_cumulative_mean = ffirst(htvv_cumulative_mean, na.rm = TRUE),
        age_at_admission = ffirst(age_at_admission, na.rm = TRUE),
        sex_category = ffirst(sex_category, na.rm = TRUE),
        hospital_id = ffirst(hospital_id, na.rm = TRUE),
        mortality_landmark = as.integer(all(mortality_enc_full == 1 & max_day == landmark_day)),  
        VFHs = ffirst(VFHs, na.rm = TRUE),
        land_mark_hour_max = fmax(hour_sequence_from_first_vent_to_discharge, na.rm = TRUE), 
        .groups = "drop"
        )  |> 
      distinct() |> 
      ungroup()
    
    # get people that died on landmark day and bind... they won't have 24 hours of data to add
    died_on_landmark_day <- model_data_prep |> 
      filter(mortality_landmark == 1 & day == landmark_day) |> 
      group_by(clif_hospitalizations_joined_id) |> 
      # take last hour
      slice_tail(n=1)
    
    model_data <- model_data_prep |> 
      # we only keep rows when the patient survived the whole day and is still intubated
      filter(land_mark_hour_max == landmark_hour & mortality_landmark == 0) |> 
      bind_rows(died_on_landmark_day) |> 
      left_join(severity_of_illness_vars) |> 
        mutate(sf_category = case_when(
    # admit_sf_ratio_worst >= 315 ~ "Normal",
    admit_sf_ratio_worst >= 235 ~ "Mild_Normal",
    admit_sf_ratio_worst >= 150 ~ "Moderate",
    !is.na(admit_sf_ratio_worst) ~ "Severe",
    TRUE ~ NA_character_
  )) %>%
  mutate(sf_category = factor(sf_category, levels = c("Severe", "Moderate", "Mild_Normal")))
    
    
    
    
    
    # Format data# Formatmin_grid.C5_rules() data
    model_data <- as.data.frame(model_data)
    model_data$sex_category <- factor(model_data$sex_category, levels = c("Male", "Female"))
    model_data$hospital_id <- as.factor(model_data$hospital_id)
    model_data$mortality_landmark <- as.factor(model_data$mortality_landmark)

    
    explanatory <- c(
      # "admit_laps2_worst", 
      "daily_sf_ratio",
      "daily_laps2", "age_at_admission", "sex_category")
    
    if (hosp_id_count > 1) {
      hosp_counts <- model_data %>%
        count(hospital_id, sf_category)
      
      sufficient_hospitals <- hosp_counts %>%
        filter(n >= 10) %>%
        group_by(hospital_id) %>%
        tally() %>% 
        filter(n == length(unique(model_data$sf_category)))  # ensure support in all SF categories
      
      # # need to have people that span all groups
      # if (nrow(sufficient_hospitals) >= 2) {
      #   model_data <- model_data |> right_join(sufficient_hospitals)
      # }
      
      # explanatory <- c(explanatory, "hospital_id")
      model_data |> distinct(hospital_id) |> pull()  
      message("✅ Including the following hospitals in the model ",
          paste(model_data |> distinct(hospital_id) |> pull(), collapse = ", "))
    }
    
    
    
    message(paste0("number of rows in this model        = ", model_data |> nrow()))
    message(paste0("number of encounters in this model  = ", model_data |>
                     select(clif_hospitalizations_joined_id) |> nrow()))
    
    
    
    if (nlevels(factor(model_data$mortality_landmark)) < 2) {
      message("Skipping mortality model for day ", landmark_hour/24, " (not enough variation in outcome)")
      next
    }
    
    ### if there are not enough patient rows we should move on and you can't run the model for that day... which is not a problem!
    if (model_data |> nrow() < 100) {
      message("Skipping mortality model for day ", landmark_hour/24, " (not enough variation in outcome)")
      next
    }
    
           # If using `htvv_cumulative * htvv_eligible - htvv_cumulative`... it means
           #    "Does the slope of cumulative HTVV (effect per hour) vary 
           #    depending on eligible hours, without assuming there's a 
           #    main effect of HTVV when eligibility is zero?"
           #    
           #    SF category does not converge for mortality
    htvv_form <- (paste("+ htvv_cumulative"))
    
    
    
    # Mortality Model
    form_mort <- paste("mortality_landmark ~", paste(explanatory, collapse = " + "), 
                       paste(htvv_form))
    fit_mort <- glm(formula = as.formula(form_mort), data = model_data, family = binomial(link = "logit"))
    tidy_mort <- broom::tidy(fit_mort, conf.int = TRUE, exponentiate = TRUE) %>%
      mutate(site = site_name, landmark_hour = landmark_hour)
    
    results_mort_list[[as.character(landmark_hour)]] <- tidy_mort
    
    # # Save mortality results
    # write.csv(tidy_mort, paste0("output/final/model_coefs_landmark_mortality_day_", landmark_hour, "__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")

    
    
    htvv_form <- (paste("+ htvv_cumulative * sf_category"))
    # VFD Model
    form_vfd <- paste("VFHs ~", paste(explanatory, collapse = " + "), 
                       # "Does the slope of cumulative HTVV (effect per hour) vary 
                       #    depending on eligible hours, without assuming there's a 
                       #    main effect of HTVV when eligibility is zero?"
                       paste(htvv_form))
    fit_vfd <- lm(formula = as.formula(form_vfd), data = model_data)
    tidy_vfd <- broom::tidy(fit_vfd, conf.int = TRUE) %>%
      mutate(site = site_name, landmark_hour = landmark_hour)
    
    results_vfd_list[[as.character(landmark_hour)]] <- tidy_vfd
    
    tidy_effects_vfd <- marginal_effects <- ggpredict(
      model = fit_vfd,  # replace with your actual model object name
      terms = c("htvv_cumulative", "sf_category")
    ) |> 
      mutate(site = site_name, landmark_hour = landmark_hour)
    
    results_effects_vfd_list[[as.character(landmark_hour)]] <- tidy_effects_vfd

    # Save VFD results
    # write.csv(tidy_vfd, paste0("output/final/model_coefs_landmark_vfds_day_", landmark_hour, "__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE)

  }
  
 # Combine and write final CSVs
  final_mort_df <- bind_rows(results_mort_list)
  final_vfd_df <- bind_rows(results_vfd_list)
  final_effects_vfd_df <- bind_rows(results_effects_vfd_list)

  write.csv(final_mort_df, paste0("output/final/ALL_model_coefs_landmark_mortality_", site_name, "__", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
  write.csv(final_vfd_df, paste0("output/final/ALL_model_coefs_landmark_vfds_", site_name, "__", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8") 
  write.csv(final_effects_vfd_df, paste0("output/final/ALL_model_coefs_landmark_effects_vfds_", site_name, "__", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
}

# Run
run_daily_landmark_analysis_for_variable_finalfit()


```


### visualizing landmark
```{r}
# Load results
mort_df <- read.csv(paste0("output/final/ALL_model_coefs_landmark_mortality_", site_name, "__", Sys.Date(), ".csv"))
vfd_df  <- read.csv(paste0("output/final/ALL_model_coefs_landmark_vfds_", site_name, "__", Sys.Date(), ".csv"))
effects_vfd_df  <- read.csv(paste0("output/final/ALL_model_coefs_landmark_effects_vfds_", site_name, "__", Sys.Date(), ".csv"))

# Filter out just the HTVV interaction term from each
mort_effect <- mort_df %>%
  filter(term == "htvv_cumulative") %>%
  select(landmark_hour, estimate, conf.low, conf.high) %>%
  mutate(
    model = "Mortality (OR)",
    estimate_type = "OR",
    sf_category = "Overall"  # assign a label instead of leaving NA
  )
...
# 
# vfd_effect <- vfd_df %>%
#   filter(term == "htvv_cumulative:sf_category") %>%
#   select(landmark_hour, estimate, conf.low, conf.high) %>%
#   mutate(
#     model = "VFHs (Coefficient)",
#     estimate_type = "Linear"
#   )


# Compute VFH marginal effects for each SF category
htvv_main <- vfd_df %>%
  filter(term == "htvv_cumulative") %>%
  select(landmark_hour, estimate_main = estimate, std_error_main = std.error)

vfd_marginal <- vfd_df %>%
  filter(str_detect(term, "htvv_cumulative:sf_category")) %>%
  mutate(sf_category = str_remove(term, "htvv_cumulative:")) %>%
  select(landmark_hour, sf_category, estimate_interaction = estimate, std_error_interaction = std.error) %>%
  left_join(htvv_main, by = "landmark_hour") %>%
  mutate(
    marginal_estimate = estimate_main + estimate_interaction,
    se_combined = sqrt(std_error_main^2 + std_error_interaction^2),
    conf.low = marginal_estimate - 1.96 * se_combined,
    conf.high = marginal_estimate + 1.96 * se_combined,
    model = "VFHs (Coefficient)",
    estimate_type = "Linear"
  ) %>%
  select(landmark_hour, sf_category, marginal_estimate, conf.low, conf.high, model, estimate_type)

# Also include the Severe group directly (uses htvv_cumulative only)
vfd_severe <- htvv_main %>%
  mutate(
    sf_category = "Severe",
    marginal_estimate = estimate_main,
    se_combined = std_error_main,
    conf.low = marginal_estimate - 1.96 * se_combined,
    conf.high = marginal_estimate + 1.96 * se_combined,
    model = "VFHs (Coefficient)",
    estimate_type = "Linear"
...
# Combine all VFH marginal effects
vfd_effect <- bind_rows(vfd_marginal, vfd_severe)

combined_effect <- bind_rows(mort_effect, vfd_effect) |> 
  filter(landmark_hour != 672)  

# Plot
ggplot(combined_effect, aes(x = landmark_hour / 24, y = marginal_estimate, color = sf_category)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.3) +
  facet_wrap(~ model, scales = "free_y") +
  geom_hline(data = tibble(model = c("Mortality (OR)", "VFHs (Coefficient)"),
                           yintercept = c(1, 0)),
             aes(yintercept = yintercept),
             linetype = "dashed", color = "gray40") +
  labs(
    title = "Effect of HTVV on Mortality and VFHs Across Landmark Days by SF Ratio",
    x = "Landmark Day",
    y = "Effect Estimate",
    color = "SF Ratio Category"
  ) +
  theme_minimal()
...
# Make sure the landmark hour is expressed in days for readability
effects_vfd_df_plot <- effects_vfd_df %>%
  mutate(landmark_day = as.numeric(landmark_hour) / 24) |> 
  filter(landmark_hour != 672)

# Plot
ggplot(effects_vfd_df_plot, aes(x = x, y = predicted, color = group)) +
  geom_line(size = 1.2) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.15, color = NA) +
  facet_wrap(~ landmark_day, scales = "free") +
  labs(
    title = "Marginal Effect of HTVV on VFHs by SF Ratio Category and Landmark Day",
    subtitle = "Adjusted for age and LAPS2",
    x = "Cumulative HTVV Hours",
    y = "Predicted Ventilator-Free Hours",
    color = "SF Ratio Category",
    fill = "SF Ratio Category"
  ) +
  theme_minimal() +
  theme(
    legend.position = "top",
...
new_data <- data.frame(cumulative_htvv_hours = htvv_levels,
                       sex_category = "female",  # Set other covariates (e.g., male)
...
Able to keep random intercept in for continuous outcomes
...
htvv_analysis <- htvv_step1 %>%
  ...
  ## currently want complete cases with PF ratio... don't need the pco2 and ph to be complete
  drop_na() %>%
...
write.csv(results_sex_category_ltvv, paste0("output/final/model_coefs_sex_category_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
```
...
write.csv(results_sex_category_height_ltvv, paste0("output/final/model_coefs_sex_category_height_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
```
...
write.csv(results_sex_interaction_height_ltvv, paste0("output/final/model_coefs_sex_interaction_height_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
```
...
write.csv(results_sex_interaction_height_ltvv_rcs, paste0("output/final/model_coefs_sex_interaction_height_RCS_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
...
write.csv(results_sex_category_bin, paste0("output/final/model_coefs_sex_category_bin_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
```
...
results_sex_category_height_bin <- model_results %>%
  ...
  mutate(
    ...
    Model = "Female + Height ~ HTVV (Binary)",
    Site = site_name
  ) %>%
  # Rearrange columns
  dplyr::select(
...
write.csv(results_sex_category_height_bin, paste0("output/final/model_coefs_sex_category_height_bin_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
```
...
write.csv(results_sex_interaction_height_bin, paste0("output/final/model_coefs_sex_interaction_height_bin_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
```
...
results_sex_interaction_height_rcs_bin <- model_results %>%
  ...
  mutate(
    ...
    Model = "Female * rcs(Height) ~ HTVV (Binary)",
    Site = site_name
  ) %>%
  # Rearrange columns
  dplyr::select(
...
write.csv(results_sex_interaction_height_rcs_bin, paste0("output/final/model_coefs_sex_interaction_height_rcs_bin_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
```
...
predictions_model11$Model <- "Female ~ Vt" 
...
write.csv(all_predictions, paste0("output/final/model_preds_sex_category_height_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE, fileEncoding = "UTF-8")
...
# Categorizing VT (Tidal Volume) per kg IBW into Bins
```{r}
...
vt_bin_data <- first_intubation_data %>%
  ...
  filter(!is.na(vt_bin)) %>%
  # mutate(vt_bin = relevel(factor(vt_bin), ref = "6-7 cc/kg")) |>
...
run_vt_bin_analysis <- function(interaction_term = FALSE) {
  ...
  vt_bin_data_patient <- vt_bin_data |> 
    ...
    group_by(clif_hospitalizations_joined_id) %>%
    dplyr::summarize(
      vt_cckg = fmean(vt_cckg, na.rm = TRUE),
      age_at_admission = ffirst(age_at_admission),
      sex_category = ffirst(sex_category),
      hospital_id = ffirst(hospital_id),
  ...
  write.csv(mortality_results, file = mortality_filename, row.names = FALSE, fileEncoding = "UTF-8")
  ...
  write.csv(vfd_results, file = vfd_filename, row.names = FALSE, fileEncoding = "UTF-8")
  ...
  write.csv(mortality_results_early, file = mortality_filename_early, row.names = FALSE, fileEncoding = "UTF-8")
  ...
  write.csv(vfd_survivor_results, file = vfd_survivor_filename, row.names = FALSE, fileEncoding = "UTF-8")
  ...
  write.csv(mortality_results_24hr, file = mortality_filename_24hr, row.names = FALSE, fileEncoding = "UTF-8")
