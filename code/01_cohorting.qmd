...
import_df <- function(x) {
  ...
  if (file_type == "parquet") {
    ...
    df <- read_function(file_path) |>
      ...
      # get everything to lower  (Moved above R.JOIN 5/2025 in case ppl have capitals in _id Variables)
      mutate(across(where(is_character), str_to_lower)) |> 
...
df_ibw <- clif_vitals |> 
  ...
  filter(height_in >= 40 & height_in <= 100) |>  # assumming these are kiddos and/or amputees which means IBW will be inaccurate
  group_by(clif_hospitalizations_joined_id) |> 
...
df_hourly_resp_support1 <- df_resp_support |> 
  ...
  # need to drop other category
  filter(device_category != "other") |> 
  ...
  mutate(
    ...
    vent_episode_id = fcumsum(
      device_category == "imv" & 
        ...
        (is.na(tracheostomy) | tracheostomy == 0) & 
...
### Currently df_hourly_resp_support 1 ... the gap_vent_hours are only preventing changing ID if there is a huge gap in documentaion between IMV rows with nothing in between.  Currently having a few hours of nasal cannula between vent would change to a new Vent ID WHEN the vent comes BACK
...
df_hourly_resp_support2 <- df_hourly_resp_support1 |> 
  ...
  mutate(vent_episode_id = as.double(vent_episode_id)) |>
  mutate(
    # change those not with trach/vent to NA
    vent_episode_id = fcase(
      device_category == "imv" | tracheostomy == 1, vent_episode_id,
      default = NA
    )) |>
  # find out who came in vented or trached on day one so we can increase enc_id by one
  group_by(clif_hospitalizations_joined_id) |>
  # did they come in with trach or vent and have episode ID = 0
  mutate(vent_episode_id_check = fcase(
    (tracheostomy == 1 |
       device_category == "imv") & vent_episode_id == 0, 1,
    default = 0
  )) |>
  mutate(vent_episode_id_check = fmax(vent_episode_id_check, na.rm = T)) |>
  ungroup() |>
  
  ## sequence up if they meet criteria above (this way we can always keep vent_episode_id == 1)
  mutate(
    vent_episode_id = fcase(
      vent_episode_id_check == 1 &
        !is.na(vent_episode_id),                vent_episode_id + 1,
      rep_len(TRUE, length(vent_episode_id)),   vent_episode_id
    )) |>
