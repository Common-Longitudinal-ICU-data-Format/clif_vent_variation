  ...
  for (var in datetime_vars) {
    ...
    if (var == "death_dttm" & !inherits(df[[var]], "POSIXct")) {
      ...
      if (!inherits(df[[var]], "POSIXct")) {
        ...
        df[[var]] <- as.POSIXct(df[[var]], tz = local_tz)
    ...
    # If it's already POSIXct, adjust to local timezone
    df[[var]] <- with_tz(df[[var]], tzone = local_tz)
  }
...
# 
# 
# write_parquet_tz(df_hourly_resp_support, paste0("output/intermediate/df_hourly_resp_support_", Sys.Date(), ".parquet"))
# 
...
save_data_frames <- function(data_frame_names, output_dir = "output/intermediate/clean_db") {
  ...
  for (df_name in data_frame_names) {
    ...
    if (exists(df_name, envir = .GlobalEnv)) {
      ...
      write_parquet_tz(df, output_file)
